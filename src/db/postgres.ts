...
client.query(
  "CREATE TABLE IF NOT EXISTS jobinputs( input_uid UUID NOT NULL PRIMARY KEY, job_title VARCHAR(50) , job_location VARCHAR(50) );",
  (err) => {
    if (err) {
      console.log(err.stack);
    }
...
client.query(
  "CREATE TABLE IF NOT EXISTS user_table( id UUID NOT NULL PRIMARY KEY, username VARCHAR(100) NOT NULL, email VARCHAR(100) NOT NULL, resumestring VARCHAR(100),resumetext VARCHAR , experience VARCHAR ,skills VARCHAR(100),age VARCHAR, gender VARCHAR,  UNIQUE(email) );",
  (err) => {
    if (err) {
      console.log(err.stack);
    }
